...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchRandomImages() {
    const numImages = 6; // Number of random images to fetch
    const imageContainer = document.getElementById("random-images");
    const accessKey = "u5KruTI5rqU6y7xJyjxrj8hHgu4amctNiiG_wuSmUEY";

    // Fetch random images from the Unsplash API
    fetch(
      `https://api.unsplash.com/photos/random?client_id=${accessKey}&count=${numImages}`
    )
      .then((response) => response.json())
      .then((data) => {
